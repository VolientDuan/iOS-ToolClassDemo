...
#define kBDKNotifyHUDDefaultRoundness    2.0f
#define kBDKNotifyHUDDefaultOpacity      0.75f
#define kBDKNotifyHUDDefaultPadding      10.0f
#define kBDKNotifyHUDDefaultPaddingImage      5.0f
#define kBDKNotifyHUDDefaultInnerPadding 5.0f
...
+ (instancetype)notifyHUDWithImage:(UIImage *)image text:(NSString *)text {
    static BDKNotifyHUD *bdNofifyHud = nil;
    static dispatch_once_t once;
    dispatch_once(&once,^{
        bdNofifyHud = [[self alloc] initWithImage:image text:text];
    });
    
    bdNofifyHud.image = image;
    bdNofifyHud.text = text;
    return bdNofifyHud;
}
...
- (UIImageView *)imageView {
    ...
    if (self.image != nil) { 
        ...
        frame.origin = CGPointMake((self.backgroundView.frame.size.width - frame.size.width) / 2, kBDKNotifyHUDDefaultPaddingImage);
...
- (void)adjustTextLabel:(UILabel *)label {
    ...
    frame.size.height = _textLabel.frame.size.height;
    _textLabel.frame = frame;
}
